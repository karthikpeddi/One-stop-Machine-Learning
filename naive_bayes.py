...
def naive_bayes1(dataset,target,dataset_file_name):
    ...
    for i in cat_var:
        if i!=target:
            if len(set(dataset[i]))<=10:
                ...
                plt.tight_layout()
                filename='static/Plots/Naive Bayes/'+dataset_file_name+'/Categorical Variables/Attribute '+i+'.png'
    ...
    if len(set(dataset[target]))<=10:
        ...
        ax.set_title('Confusion Matrix of Trained Naive Bayes Classifier',loc='center')
        ax.set_xlabel('Actual target value')
        ax.set_ylabel('Predicted Value')
        filename='static/Plots/Naive Bayes/'+dataset_file_name+'/Metrics/confusion_matrix.png'
        conf_mat.append(filename.replace(" ","%20"))
        plt.savefig('static/Plots/Naive Bayes/'+dataset_file_name+'/Metrics/confusion_matrix.png',dpi=1000)
        plt.cla()
        plt.close()
    ...
    def plot_boxplot(dataset,cat_var,target):
        ...
        py.plot(fig, filename='static/Plots/Naive Bayes/'+dataset_file_name+'/Numerical Variables/Boxplot of numerical variables.html',auto_open=False)
        return
    plot_boxplot(dataset,cat_var,target)
    num_plots=[]
    filename1='static/Plots/Naive Bayes/'+dataset_file_name+'/Numerical Variables/Boxplot of numerical variables.html'
